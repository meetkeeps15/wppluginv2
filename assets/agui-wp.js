(function(){
  ...
  async function generateImage(prompt){
    ...
    let endpoints = [];
    const wpEndpoint = (cfg.wpImageEndpoint || '/wp-json/agui-chat/v1/image/generate').replace(/\/$/, '');
    const agentEndpoint = (cfg.agentImageEndpoint || '').replace(/\/$/, '');
    const fastApiBase = (cfg.fastApiBase || '').replace(/\/$/, '');
    const fastApiEndpoint = fastApiBase ? `${fastApiBase}/api/fal/generate` : (cfg.fastApiImageEndpoint || '').replace(/\/$/, '');

    // Prefer WordPress first, then agent-server (with toggled scheme), then FastAPI
    ...
    if(fastApiEndpoint) endpoints.push(fastApiEndpoint);

    // Feature flag: allow disabling non-WordPress fallbacks entirely
    const disableFallbacks = !!(cfg.disableAgentFallbacks || cfg.disableNonWpFallbacks);
    if(disableFallbacks) {
    ...
    const timeoutFor = (endpoint, idx=0) => {
      try {
        const u = new URL(endpoint, window.location.origin);
        const isWp = u.pathname.startsWith('/wp-json/agui-chat');
        const isAgent = u.hostname.includes('ngrok') || u.pathname.includes('/api/');
        const isFastApi = /\/api\/fal\//.test(u.pathname);
        let base = isWp ? 25000 : isAgent ? 45000 : isFastApi ? 30000 : 20000;
        return base + Math.min(idx, 3) * 7000;
      } catch(_) { return 20000; }
    };
    ...
    for (let i=0; i<endpoints.length; i++){
      ...
      try{
        ...
        // ngrok header for agent endpoints
        try {
          const u = new URL(endpoint, window.location.origin);
          const isAgent = u.hostname.includes('ngrok') || u.pathname.includes('/api/');
          if (isAgent) headers['ngrok-skip-browser-warning'] = '1';
        } catch(_) {}
        const controller = new AbortController();
        const tm = setTimeout(()=> controller.abort(), timeoutFor(endpoint, i));
        const resp = await fetch(endpoint, { method:'POST', headers, body: JSON.stringify({ prompt }), signal: controller.signal});
        clearTimeout(tm);
        if(!resp.ok){ console.warn('Image generation not ok', resp.status, resp.statusText, 'via', endpoint); continue; }
        const ct = (resp.headers.get('content-type')||'').toLowerCase();
        if(ct.includes('application/json')){
          const data = await resp.json().catch(()=>({}));
          const url = normalizeImageUrl(extractImageUrl(data));
          if(url){ renderImage(url, prompt); return; }
        } else {
          const txt = await resp.text().catch(()=>(''));
          const url = normalizeImageUrl(txt);
          if(url){ renderImage(url, prompt); return; }
        }
        console.warn('No valid image returned via', endpoint);
      } catch(e){ 
        if (e && (e.name === 'AbortError' || String(e).includes('aborted'))) {
          console.warn('Image generation timed out via', endpoint, '— will try next endpoint.');
        } else {
          console.error('Image generation error via', endpoint, e);
        }
        continue;
      }
    }
...
(function(){
  ...
  const state = { name:'', email:'', description:'', icon:'gauge', brand_text:'', slides:[], selected:null, styles:[], nameInputBound:false };
  ...
  const form1 = steps.s1; if(form1){
    form1.addEventListener('submit', async function(ev){ ev.preventDefault();
      ...
      if(btn){ btn.disabled = false; btn.textContent = 'Get Started'; }
      showStep('s2');
      setTimeout(()=>{ const sp=q('.bmms-splash', steps.s2); if(sp) sp.style.display='none'; const ops=q('.bmms-options', steps.s2); if(ops){ ops.style.display=''; ops.classList.add('show'); try{ ops.style.pointerEvents = 'auto'; ops.style.opacity = '1'; const btns = Array.from(ops.querySelectorAll('.bmms-style-btn')); btns.forEach(b=>{ try{ b.style.pointerEvents = 'auto'; b.setAttribute('tabindex','0'); }catch(_){} }); }catch(_){} } }, 900);
    });
  }
    ...
    s2.addEventListener('click', function(e){
      ...
      if(t.closest('.bmms-next')){
        ...
        setTimeout(()=>{
          ...
          // Bind brand name input events once
          const inp = q('input[name="brand_text"]', steps.s3);
          if(inp && !state.nameInputBound){
            let to=null;
            inp.addEventListener('input', function(){
              if(to) clearTimeout(to);
              to = setTimeout(()=>{ if(inp.value.trim().length>=1) generateNames(); }, 350);
            });
            inp.addEventListener('keydown', function(ev){ if(ev.key==='Enter'){ ev.preventDefault(); generateNames(); } });
            state.nameInputBound = true;
          }
          // Ensure a status line exists
          const fields = q('.bmms-fields', steps.s3);
          if(fields){ let st = q('#bmms-name-status', steps.s3); if(!st){ st=document.createElement('div'); st.id='bmms-name-status'; st.style.cssText='margin-top:6px;color:#6b7280;font-size:12px;'; fields.appendChild(st); } }
          // Ensure a suggestions host exists
          try {
            const fieldsEl = q('.bmms-fields', steps.s3);
            if (fieldsEl) {
              let host = q('.bmms-name-options', steps.s3);
              if (!host) {
                host = document.createElement('div');
                host.className = 'bmms-name-options';
                host.style.cssText = 'display:none; flex-wrap:wrap; gap:8px; margin-top:8px;';
                fieldsEl.appendChild(host);
              }
            }
          } catch(e){}
          // Hide any manual generate button; auto-suggest is enabled
          try{ const genBtn = q('.bmms-generate', steps.s3); if(genBtn){ genBtn.style.display='none'; } }catch(_){}
          // If jQuery is available, add equivalent input binding
          try {
            if (window.jQuery) {
              const $ = window.jQuery;
              const $inp = $(q('input[name="brand_text"]', steps.s3));
              if ($inp && $inp.length) {
                let dTo = null;
                $inp.off('input.agui').on('input.agui', function(){
                  if (dTo) clearTimeout(dTo);
                  const v = String($(this).val()||'').trim();
                  dTo = setTimeout(function(){ if(v.length>=1) generateNames(); }, 350);
                });
                $inp.off('keypress.agui').on('keypress.agui', function(ev){ if(ev.key==='Enter'){ ev.preventDefault(); generateNames(); } });
              }
            }
          } catch(e){}
          // Auto-generate suggestions when arriving, if we have context
          try{
            const cur = q('input[name="brand_text"]', steps.s3)?.value?.trim()||'';
            const hasCtx = (desc && desc.value) || ((state.styles||[]).length);
            if(hasCtx && !cur){ generateNames(); }
          }catch(_){ }
        }, 900);
      }
    });
  ...
  const s3 = steps.s3; if(s3){
    ...
    function setNameStatus(msg, type){
      const st = q('#bmms-name-status', s3);
      if(!st) return;
      if(!msg){ st.textContent=''; st.style.display='none'; return; }
      st.style.display='';
      st.textContent = msg;
      st.style.color = (type==='error') ? '#dc2626' : (type==='warn') ? '#b45309' : '#6b7280';
    }
    async function generateNames(){
      // Read latest inputs
      const desc = (q('textarea[name="brand_desc"]', s3)?.value?.trim() || '').toLowerCase();
      const current = (q('input[name="brand_text"]', s3)?.value?.trim() || '');
      // Strongly anchor suggestions to ALL words the user typed
      const curTokens = (current || '').toLowerCase().match(/[a-z0-9]+/g) || [];
      const curWords = (current||'').trim().split(/\s+/).filter(Boolean);
      const uniqueTokens = Array.from(new Set(curTokens)).filter(Boolean);
      const curRoot = uniqueTokens.length ? uniqueTokens[uniqueTokens.length-1] : '';
      const baseWords = ((state.description||'') + ' ' + desc).toLowerCase().match(/[a-z]{3,}/g) || [];
      const roots = uniqueTokens.length ? uniqueTokens.slice(0,3) : [curRoot].concat(baseWords.filter(Boolean)).slice(0,3).filter(Boolean);
      const cap = s => s ? s.charAt(0).toUpperCase() + s.slice(1) : '';
      const host = q('.bmms-name-options', s3);
      if(host){ host.style.display=''; host.innerHTML = '<div style="padding:8px;color:#6b7280">Generating name ideas…</div>'; }

      // Prefer WordPress endpoint if available
      const cfg = window.AGUiConfig || window.AGConfig || {};
      const endpoint = (cfg.wpBrandnameEndpoint || '/wp-json/agui-chat/v1/brandname/generate');
      try{
        const payload = { description: desc || state.description || '', styles: (state.styles||[]), current };
        const resp = await fetch(endpoint, { method:'POST', headers:{ 'Content-Type':'application/json', 'ngrok-skip-browser-warning':'true' }, body: JSON.stringify(payload) });
        const j = await resp.json().catch(()=>({}));
        if(resp.ok){
          let names = (j?.data?.suggestions || j?.suggestions || []).map(s=> typeof s==='string' ? s : s?.name).filter(Boolean);
          // Deduplicate and rank by simple relevance to description/styles
          const styles = (state.styles||[]).map(s=> String(s).toLowerCase());
          const score = (name)=>{
            let sc = 0; const lower = String(name||'').toLowerCase();
            // Weight matches to any token the user typed; bonus if multiple match
            let matchedTokens = 0;
            uniqueTokens.forEach(tok=>{ if(tok && lower.includes(tok)) { sc += 4; matchedTokens++; } });
            if(matchedTokens >= 2) sc += 2; // multi-token relevance bonus
            roots.forEach(r=>{ if(r && lower.includes(String(r).toLowerCase())) sc+=2; });
            styles.forEach(s=>{ if(lower.includes(s)) sc+=2; });
            if(lower.length <= 12) sc+=1; // brevity preference
            if(/^[A-Za-z][A-Za-z]+$/.test(name)) sc+=1; // clean word form
            return sc;
          };
          names = Array.from(new Set(names));
          names.sort((a,b)=> score(b)-score(a));
          setNameStatus('Connected: WordPress name suggestions','ok');
          renderNameOptions(names.slice(0,6));
          return;
        } else {
          // Suppress server error status in UI; fallback will render suggestions silently
          setNameStatus('', '');
        }
      }catch(err){ console.warn('Brandname endpoint failed, using local generator.', err); setNameStatus('', ''); }

      // Fallback: Local generator anchored to current input with style influence
      const root = curRoot || (roots[0] || 'brand');
      const R = cap(root);
      const suffixes = ['ly','ify','io','ora','ity','ster','zen','sy','za','zo']; // short brandy suffixes
      const nouns = ['Labs','Works','Co','Hub','Craft','Prime','Nova','Forge','Pulse','Nest','Studio','Flow','Spark','Mint','Rise'];
      const prefixes = ['Neo','Pro','Ultra','Go','My','True','Peak','Alpha','Vivid','Aero','Zen','Clear','Brand','Vibe'];
      const styleInfluence = (state.styles||[]).map(s=> cap(String(s).replace(/\s+/g,'')));
      const combos = new Set();
      if(host){ host.innerHTML=''; }
      // Generate variants for each token the user typed (up to 3 tokens)
      const tokenCaps = (uniqueTokens.length ? uniqueTokens : [root]).slice(0,3).map(cap);
      tokenCaps.forEach(T => {
        // Morphological variants
        suffixes.forEach(sfx=> combos.add(T + sfx));
        // Corporate-style nouns
        nouns.forEach(n=> combos.add(T + n));
        // Prefix combos
        prefixes.forEach(p=> combos.add(p + T));
        // Style-influenced
        styleInfluence.forEach(s=> combos.add(s + T));
        // Vowel-light variant
        const vowelless = T.replace(/[aeiou]/ig,'');
        if(vowelless.length>=2) combos.add(vowelless + 'io');
      });

      // Multi-word merges (pairwise and triple)
      if(curWords.length >= 2){
        const caps = curWords.map(w=> cap(w));
        // Pairwise concatenations
        for(let i=0;i<caps.length;i++){
          for(let j=i+1;j<caps.length;j++){
            const pair = caps[i] + caps[j];
            combos.add(pair);
            nouns.forEach(n=> combos.add(pair + ' ' + n));
          }
        }
        // Triple concatenation and with noun
        if(caps.length >= 3){
          const triple = caps[0] + caps[1] + caps[2];
          combos.add(triple);
          combos.add(triple + ' Labs');
        }
        // Acronym + noun
        const acronym = caps.map(c=> c[0]).join('');
        if(acronym.length >= 2){ combos.add(acronym.toUpperCase()); combos.add(acronym.toUpperCase() + ' Studio'); }
      }

      const list = Array.from(combos)
        .map(s=> s.replace(/\s+/g,' ').trim())
        .filter(n=> n.length<=18)
        .slice(0,12);
      if(list.length === 0){
        if(host){ host.innerHTML = '<div style="padding:8px;color:#dc2626">Unable to generate suggestions. Please add a short brand description or try again.</div>'; }
        setNameStatus('Unable to generate suggestions','error');
        return;
      }
  ...
  async function buildVariations(){
    ...
    const totalN = 6;
    // Show loading state with animated counter
    wrap.innerHTML = '<div style="text-align:center; padding: 40px; color: #6b7280;">'
      + 'Generating AI logos… <span id="bmms-progress-count">0</span>/<span id="bmms-progress-total">'+totalN+'</span>'
      + '<div style="margin-top:10px; height:6px; background:#eee; border-radius:3px; overflow:hidden;">'
        + '<div id="bmms-progress-bar" style="width:0%; height:100%; background:#6366f1; transition: width 0.25s ease;"></div>'
      + '</div>'
    + '</div>';
    const countEl = q('#bmms-progress-count', s4);
    const barEl = q('#bmms-progress-bar', s4);
    
    try {
      let generated = 0;
      let variants = await createVariants(totalN, state, (count, total)=>{
        generated = count;
      ...
      // Group variants into slides of 3 cards each
      const groupSize = 3;
      const slides = [];
      for(let i=0;i<total;i+=groupSize){ slides.push(variants.slice(i, i+groupSize)); }
      state.slides = slides;
      state.selected = null;
      let idx = 0;
  ...
  async function createVariants(n, st, onProgress){ 
    ...
    const basePrompt = `Create a professional logo for "${name}". Business description: ${description}. Style: ${iconStyle}.` + (styleSeeds ? ` ${styleSeeds}` : '') + emailPart + ((st && st.selectedIdea) ? ' Use the provided reference image as the foundation for layout and iconography.' : '');
    ...
    // Multi-endpoint fallback: prefer WordPress REST first, then Agent (incl. toggled scheme), then FastAPI
    const cfg = window.AGUiConfig || window.AGConfig || {};
    const wp = (cfg.wpImageEndpoint || '/wp-json/agui-chat/v1/image/generate').replace(/\/$/, '');
    const agent = (cfg.agentImageEndpoint || '').replace(/\/$/, '');
    ...
    const toggledAgent = (function(){
      try {
        if (!agent) return '';
        const u = new URL(agent, window.location.origin);
        return (u.protocol === 'http:' ? 'https:' : 'http:') + '//' + u.host + (u.pathname || '') + (u.search || '');
      } catch(e) { return ''; }
    })();
    let endpoints = [wp, agent, toggledAgent, fastApi].filter(Boolean);
    ...
    const adaptiveTimeout = (ep, i) => {
      try {
        const u = new URL(ep, window.location.origin);
        const isWp = u.pathname.startsWith('/wp-json/agui-chat');
        const isAgent = u.hostname.includes('ngrok') || u.pathname.includes('/api/');
        const isFastApi = /\/api\/fal\//.test(u.pathname);
        let base = isWp ? 22000 : isAgent ? 38000 : isFastApi ? 26000 : 20000; // tighter for speed
        return base + Math.min(i, 3) * 5000;
      } catch(e){ return 20000; }
    };

    const chooseSteps = (ep, i) => {
      try {
        const u = new URL(ep, window.location.origin);
        const isWp = u.pathname.startsWith('/wp-json/agui-chat');
        const isAgent = u.hostname.includes('ngrok') || u.pathname.includes('/api/');
        const isFastApi = /\/api\/fal\//.test(u.pathname);
        // Slightly fewer steps to speed up; earlier variants get even fewer for quick feedback
        const base = isWp ? 10 : isAgent ? 8 : isFastApi ? 10 : 10;
        return Math.max(6, base - (i < 3 ? 2 : 0));
      } catch(e){ return 10; }
    };
    ...
    const genOne = async (i) => {
      ...
      for (const endpoint of endpoints){
        try {
          ...
          const controller = new AbortController();
          const timeoutMs = adaptiveTimeout(endpoint, i);
          ...
          const payload = {
            ...
            format: 'png',
            guidance_scale: 3.0,
            num_inference_steps: chooseSteps(endpoint, i)
          };
          ...
          if (response.ok) {
            ...
            // Reject SVG placeholders; try next endpoint for a real generated image
            if (clean && String(clean).startsWith('data:image/svg')){
              console.warn('Ignoring SVG placeholder for variant', i, 'via', endpoint);
            } else if (clean) {
              results[i] = { id: 'ai_v' + i, dataUrl: clean, prompt: variations[i] };
              success = true;
          ...
          if (error && (error.name === 'AbortError' || String(error).includes('aborted'))) {
            console.warn('AI generation timed out via', endpoint, 'for variant', i, '— trying next endpoint.');
          } else {
      ...
      if (success) {
        completed++;
        if (typeof onProgress === 'function') { try { onProgress(completed, total); } catch(e){} }
      }
      return success;
    };

    const limit = Math.max(1, Number(cfg.variantConcurrency || 3));
    const tasks = new Array(total).fill(0).map((_,i)=> ()=> genOne(i));
    const workers = new Array(Math.min(limit, tasks.length)).fill(0).map(async function run(){
      while (tasks.length){
  ...
  function extractImageUrl(result){
    try {
      ...
      candidates.push(data.base64);
      candidates.push(data?.images?.[0]?.url);
      candidates.push(data?.response?.images?.[0]?.url);
      candidates.push(data?.output?.images?.[0]?.url);
      candidates.push(data?.result?.images?.[0]?.url);
      candidates.push(data?.data?.images?.[0]?.url);
      candidates.push(data?.data?.image_url);
